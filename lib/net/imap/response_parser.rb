...
(?# 7:  RPAR    )(\)))/ni
      ...
      def sequence_set
        if Patterns::SEQUENCE_SET_STR.match?(str = atom)
          SequenceSet.new(str)
        else
      ...
      def tagged_ext_comp
        vals = []
        while true
          case lookahead!(*ASTRING_TOKENS, LPAR).symbol
          when T_LPAR
            lpar; vals << tagged_ext_comp; rpar
          when T_NUMBER
            vals << number # special-case: returns an Integer, not a string
          else
            vals << astring
          end
          SP? or break
      ...
      def tagged_ext_simple
        case lookahead!(*ATOM_TOKENS).symbol
      ...
      #                       "(" [tagged-ext-comp] ")"
      def tagged_ext_val
        if lpar? then _ = tagged_ext_comp; rpar; _
        else              tagged_ext_simple
        end
      end
      ...
      def mailbox_list
        true and flags = parens__mbx_list_flags
        SP!  and delim = nquoted_char
        SP!  and name  = mailbox
        SP?  and ext   = mbox_list_extended
        MailboxList.new(flags, delim, name, ext)
      end
      ...
      def status_att_val
        ...
        val =
          ...
          when "APPENDLIMIT"   then number_or_nil       # RFC7889
          when "MAILBOXID"     then parens__objectid    # RFC8474
          else number? || tagged_ext_val                # future extensions
          end
        [key, val]
      end
